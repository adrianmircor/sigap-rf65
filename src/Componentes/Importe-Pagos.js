...
class ImportePagos extends React.Component {
        ...
        if (pagos.length > 0) {
          pagos.forEach((pago) => {
            if (pago.id_tipo_recaudacion != 0) {
              ...
              if (pago.validado === true) {
                pagosEnsenanzaV.push(pago);
              }
              this.setState((state) => {
                return {
                  pagosEnsenanza: [...state.pagosEnsenanza, pago],
                  pagosEnsenanzaValidos: pagosEnsenanzaV,
                };
              });
            }
            // else {
        ...
        if (pagos.length > 0) {
          pagos.forEach((pago) => {
            if (pago.id_tipo_recaudacion != 0) {
              ...
              if (pago.validado === true) {
                pagosOtrosV.push(pago);
              }
              this.setState((state) => {
                return {
                  pagosOtros: [...state.pagosOtros, pago],
                  pagosOtrosValidos: pagosOtrosV,
                };
              });
              // this.setState((state) => {
              //   return {
  ...
  CalcularImporteDos() {
    ...
    let pagoDerechoEnseñanza = this.state.pagosEnsenanza;
    let importe = 0;
    for (var indice in pagoDerechoEnseñanza) {
      if (
        pagoDerechoEnseñanza[indice].moneda == "108" ||
        pagoDerechoEnseñanza[indice].moneda == "113"
      )
